...
def export_jobs_digest(conn: sqlite3.Connection, since_date: str, min_relevance: float = 7.0) -> str:
    """Generate a markdown digest of recent relevant jobs.

    Args:
        conn: Database connection.
        since_date: ISO date string (YYYY-MM-DD) to filter from.
        min_relevance: Minimum relevance score to include.
    """
    from beacon.db.jobs import get_new_jobs_since

    jobs = get_new_jobs_since(conn, since_date, min_relevance)
    now = datetime.now().strftime("%Y-%m-%d")

    lines = [
        f"# Job Digest — {now}",
        "",
        f"> Jobs since {since_date} with relevance >= {min_relevance}",
        "",
    ]
    ...
    for j in jobs:
        company_counts[j["company_name"]] = company_counts.get(j["company_name"], 0) + 1

    lines.append(f"**{len(jobs)} relevant jobs** across **{len(company_counts)} companies**")
    lines.append("")

    # Group by company
    current_company = None
    for j in jobs:
        if j["company_name"] != current_company:
            current_company = j["company_name"]
            lines.append(f"## {current_company} ({company_counts[current_company]} jobs)")
            lines.append("")

        score_str = f"{j['relevance_score']:.1f}"
        location = j["location"] or "N/A"
        url_part = f" — [Apply]({j['url']})" if j["url"] else ""
        lines.append(f"- **[{score_str}]** {j['title']} ({location}){url_part}")

    lines.extend(["", f"*Generated by Beacon on {now}*"])
    return "\n".join(lines)


def export_jobs_report(conn: sqlite3.Connection) -> str:
    """Generate a full markdown report of all active job listings."""
    from beacon.db.jobs import get_jobs

    jobs = get_jobs(conn, status="active", limit=500)
    now = datetime.now().strftime("%Y-%m-%d")

    lines = [
        f"# Beacon Job Listings Report — {now}",
        "",
    ]

    # Summary stats
    total = len(jobs)
    relevant = sum(1 for j in jobs if j["relevance_score"] >= 7)
    company_set = {j["company_name"] for j in jobs}

    lines.extend([
        f"**{total} active jobs** across **{len(company_set)} companies** ({relevant} highly relevant)",
        "",
        "| # | Company | Title | Relevance | Location | Status |",
        "|---|---------|-------|-----------|----------|--------|",
    ])

    for i, j in enumerate(jobs, 1):
        title = j["title"][:50]
        location = (j["location"] or "N/A")[:20]
        lines.append(
            f"| {i} | {j['company_name']} | {title} | {j['relevance_score']:.1f} | {location} | {j['status']} |"
        )
