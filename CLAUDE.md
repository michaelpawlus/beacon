...
| `beacon career market` | Quarterly role-market radar: snapshot the FDE/applied-AI role family's salary, skill mix, seniority mix, recurring-role basket + what changed since last quarter | `--archetype KEY` (default `solutions_fde`) `--web/--no-web` (default web on) `--since-days N` `--list` `--vault` `--output PATH` `--dry-run` `--json` |
| `beacon career guide` | Living FDE success guide: a regenerable playbook synthesizing what you're strong at and where to spend more time, from wins + target gaps + market direction + reading queue | `--since DATE\|Nd` (default 90d) `--archetype KEY` (default `solutions_fde`) `--refresh` `--vault` `--output PATH` `--json` |

**`career guide` — living success playbook (WS5, #45).** Deterministic sectioned-markdown assembly (the `interview_brief.py` pattern — no mandatory LLM call), grounded in actual data rather than a static essay. Seven sections, each degrading gracefully when its source table is empty (useful from week one, richer every quarter): (1) **Thesis** — the value-overhang framing (static); (2) **This quarter's win mix** — `wins` table diffusion-vs-delivery split via `category_mix`; (3) **Strengths** — the deliberate mirror of gaps: advanced/expert skills + win-backed (resume-ready) skills, with win-evidence skills the profile doesn't even list yet flagged for adding; (4) **Open target gaps** — `analyze_target_gaps` (#43), horizon-weighted; (5) **Market direction** — the latest persisted `role_market_snapshots` diff (#44: comp band, rising/falling skills, stale-snapshot nudge at >90 days); (6) **Recommended focus** — heuristic synthesis (`recommend_focus`) of 2–4 do-this-next calls: e.g. *diffusion ratio skews delivery + 'evals' rising in market + open gap on evals → prioritize eval work*; market-rising-skills-that-are-also-gaps rank highest, gaps with accruing win evidence prompt a resume add; (7) **Reading & sharing queue** — high-signal `media_log` rows (rating ≥4 / shareable / `why_it_matters` set), thesis-tagged (fde/adoption/enablement/…) first. `--refresh` re-syncs aspirational target gaps into `skill_gaps` before assembling; `--vault` writes the brief to `$OBSIDIAN_VAULT_PATH/Job Search/Success Guide/` via `oj capture` (`type: success-guide`); the guide consumes the *latest persisted* market snapshot, so run `beacon career market` to refresh that input. Implementation: `beacon/materials/success_guide.py`.

**`career market` — role-market radar (WS4, absorbs #21).** Clones the `presence radar` two-input pattern for the demand side: (1) an **internal sweep** that aggregates the user's own `job_listings` across *all titles that mean roughly the same job* — "Forward Deployed Engineer", the user's own "Forward Deployed AI Solutions Lead", "Solutions Architect", "Applied AI Engineer", etc. all collapse into one **role family** via `family_for_title` (alias table in `beacon/market.py`) — and computes headcount (growth/shrink proxy), average salary, skill-frequency mix, and seniority mix; (2) an **external radar** (`--web`, default on) LLM web search for live hiring direction, comp signals, and rising/falling skills, with the same graceful degradation as the posting radar (warning + exit 0 when `ANTHROPIC_API_KEY`/network are unavailable). Two headline outputs: a **basket** of recurring exemplar roles (Palantir + frontier-lab FDEs, sourced from active `role_targets`) whose comp band is re-checked each quarter, and `diff_vs_previous` — which skills are **rising / falling / newly appearing / dropping out** of the family's JDs and how the salary bands + headcount moved since the last snapshot. Every run persists one `role_market_snapshots` row (the comp/skill/headcount time series); `--vault` writes the brief to `$OBSIDIAN_VAULT_PATH/Job Search/Role-Market/` via `oj capture` (`type: role-market`), `--dry-run` computes without persisting, `--list` prints the role families. Families: `solutions_fde` (default), `agentic`, `ai_engineer_llmops`. The dashboard nudges a quarterly run once the newest snapshot is >90 days old (or absent).
...
# Regenerate the living FDE success guide (strengths + where to spend more time)
beacon career market --no-web --json          # refresh the market input first
beacon career guide --refresh --vault --json  # synthesize wins + gaps + market
